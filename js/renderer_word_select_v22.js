(() => {
  ...
  let wordsDict = [];
  let currentPage = 0;
  let zoom = Number(localStorage.getItem('wordSelectV22Zoom') || '0');
  ...
  function updateSelectionInfo() {
    ...
    if (!selectedWords.length) {
      selectionInfo.textContent = 'v22: visualIndex = строки сверху вниз, слова слева направо';
      return;
    }
  ...
  async function main() {
    const doc = await loadJson('data/pages.word_select_v22.delta.json');
    pages = doc.p || [];
  ...
  $('btnFit').addEventListener('click', () => {
    zoom = 0;
    localStorage.setItem('wordSelectV22Zoom', String(zoom));
