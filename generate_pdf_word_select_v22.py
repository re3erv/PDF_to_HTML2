"""
V22: original AVIF page images + selectable word boxes only.
...
def clean_output_dirs() -> None:
    ...
    cleanup = [
        DATA_DIR / "pages.word_select_v22.readable.json",
        DATA_DIR / "pages.word_select_v22.abs.json",
        DATA_DIR / "pages.word_select_v22.abs.json.gz",
        DATA_DIR / "pages.word_select_v22.abs.json.zst",
        DATA_DIR / "pages.word_select_v22.delta.json",
        DATA_DIR / "pages.word_select_v22.delta.json.gz",
        DATA_DIR / "pages.word_select_v22.delta.json.zst",
        DATA_DIR / "manifest.word_select_v22.json",
        LOCALES_DIR / "words_v22.json",
    ]
    for path in cleanup:
...
def readable_pages_json(pages_meta: list[dict[str, Any]], pages_words: list[list[dict[str, Any]]]):
    ...
    return {"v": 22, "mode": "readable", "pages": pages}
...
def absolute_encoding(pages_meta: list[dict[str, Any]], pages_words: list[list[dict[str, Any]]], word_to_idx: dict[str, int]):
    ...
    return {"v": 22, "mode": "abs", "d": {"words": list(word_to_idx.keys())}, "p": pages}
...
def delta_encoding(pages_meta: list[dict[str, Any]], pages_words: list[list[dict[str, Any]]], word_to_idx: dict[str, int]):
    ...
    return {"v": 22, "mode": "delta", "d": {"words": list(word_to_idx.keys())}, "p": pages}
...
def main() -> None:
    ...
    print("V22: selectable word boxes only")
    ...
    variants = {
        "pages.word_select_v22.readable.json": readable_pages_json(pages_meta, pages_words),
        "pages.word_select_v22.abs.json": absolute_encoding(pages_meta, pages_words, word_to_idx),
        "pages.word_select_v22.delta.json": delta_encoding(pages_meta, pages_words, word_to_idx),
    }
    ...
    write_json(LOCALES_DIR / "words_v22.json", [" ".join(w["t"] for w in words) for words in pages_words], pretty=True)
    ...
    write_json(DATA_DIR / "manifest.word_select_v22.json", manifest, pretty=True)
    ...
    print()
    print("=" * 92)
    print("V22 selectable word boxes завершён")
    print("-" * 92)
    ...
    abs_zst = sizes["pages.word_select_v22.abs.json"]["zst"]
    delta_zst = sizes["pages.word_select_v22.delta.json"]["zst"]
    print()
    ...
    print("Открыть:")
    print("  http://localhost:8000/index_word_select_v22.html")
